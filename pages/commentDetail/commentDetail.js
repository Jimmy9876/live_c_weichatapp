...
Page({
  ...
  //   // queryComment.equalTo("is_hide", "1");
  ...
  onShareAppMessage: function () {
    if (that.data.isPublic == true) {
      return {
        ...
        path: '/pages/commentDetail/commentDetail?moodId=' + optionId,
      }
    }
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          // 删除此心情后返回上一页
          var Comment = Bmob.Object.extend("Comments");
          var queryDiary = new Bmob.Query(Comment);
          queryDiary.include("mood");
          queryDiary.get(optionId, {
            success: function (result) {
              // console.log("optionid:"+optionId);
            var commentnum = result.get("mood").get("commentNum");
            var id = result.get("mood").id;
            // console.log("id:"+id);
              ...
              result.destroy({
                success: function (myObject) {
                  ...
                  common.dataLoading("删除成功", "success", function () {
                    ...
                    // console.log("d=" + diaryId);
                    // console.log("1");
                    //查询单条数据，第一个参数是这条数据的objectId值
                    query.get(id, {
                      success: function (object) {
                        var a=object.get("commentNum");
                        // console.log("a="+a);
                        object.set('commentNum', commentnum - 1);
                        object.save();
                        // console.log("2");
                        // that.onShow();
                      },
